...
class ProjectionHead(nn.Module):
    def __init__(
        ...
        self.gelu = nn.GELU()
        self.fc = nn.Linear(self.projection_dim, projection_dim)
...
# VibraCLIP (Graph + IR + Raman) Lightning
class VibraCLIP(pl.LightningModule):
    def __init__(
        ...
        self.raman_encoder = SpectraEncoder(
            input_dim=spectra_encoder_input_dim,
            hidden_dim=spectra_encoder_hidden_dim,
            n_layers=spectra_encoder_n_layers,
            out_features=spectra_encoder_out_features,
        ...
        self.raman_projection = ProjectionHead(
            embedding_dim=spectra_encoder_out_features_mass,
            projection_dim=projection_latent_dim,
            dropout=projection_dropout,
            p_dropout=projection_p_dropout,
        ...
        self.loss_allpairs = loss_allpairs
    ...
    def forward(self, data):
        ...
        raman_spectra_input = data.raman_spectra.view(
            data.batch_size, self.hparams.spectra_encoder_input_dim
        )
        # IR and Raman Spectra Encoders
        ir_features = self.ir_encoder(ir_spectra_input)
        raman_features = self.raman_encoder(raman_spectra_input)
        # Molecular Mass
        if self.molecular_mass:
            ...
            raman_features = torch.cat([raman_features, mol_mass], dim=1)
        # Projection Heads
        graph_embeddings = self.graph_projection(gnn_features)
        ir_spectra_embeddings = self.ir_projection(ir_features)
        raman_spectra_embeddings = self.raman_projection(raman_features)
        return (
            graph_embeddings,
            ir_spectra_embeddings,
            raman_spectra_embeddings,
        )
    ...
    def configure_optimizers(self):
        ...
        parameters = [
            ...
            {"params": self.raman_encoder.parameters(), "lr": self.spectra_lr},
            {
                "params": itertools.chain(
                    self.graph_projection.parameters(),
                    self.ir_projection.parameters(),
                    self.raman_projection.parameters(),
                ),
                "lr": self.head_lr,
    ...
    def _loss_function(
        self, graph_embeddings, ir_spectra_embeddings, raman_spectra_embeddings
    ):
        """Loss function that aligns Graph, IR and Raman Embeddings"""
        # Compute logits
        logits_ir_graph = (
        ...
        # Losses
        graph_loss_ir = (-targets.T * self.log_softmax(logits_ir_graph.T)).sum(
            1
        )
        graph_loss_raman = (
            -targets.T * self.log_softmax(logits_raman_graph.T)
        ).sum(1)
        ir_spectra_loss = (-targets * self.log_softmax(logits_ir_graph)).sum(1)
        raman_spectra_loss = (
            -targets * self.log_softmax(logits_raman_graph)
        ).sum(1)
        # Average
        avg_loss = (
            graph_loss_ir
        ...
        return (
            ...
            ir_spectra_loss,
            raman_spectra_loss,
            None,  # There is no IR-Raman alignment
        )

    def _loss_function_allpairs(
        self, graph_embeddings, ir_spectra_embeddings, raman_spectra_embeddings
    ):
        ...
        ir_raman_loss = (-targets * self.log_softmax(logits_ir_raman)).sum(1)
    ...
    def training_step(self, data, batch_idx):
        ...
        # Collect general loss for training
        ...
        self.log(
            "train_raman_loss",
            raman_spectra_loss.mean(),
            on_step=False,
            on_epoch=True,
            prog_bar=True,
        )
        # Only if allpairs loss
        if self.loss_allpairs:
            self.log(
                "train_ir_raman_loss",
                ir_raman_loss.mean(),
    ...
    def validation_step(self, data, batch_idx):
        ...
        # Collect general loss for validation
    ...
    def test_step(self, data, batch_idx):
        ...
        # Collect general loss for testing
